/*Say you are asked to retrieve all employee records 
...
it has not been retrieved from the data. 
Therefore, the condition will use a sub-query to retrieve the average salary information 
...
which can then be used to query specific information. 
...
(SELECT SALARY FROM EMPLOYEES ORDER BY SALARY DESC LIMIT 5) AS SALARY_TABLE;
...
SELECT * FROM EMPLOYEES WHERE YEAR(FROM_DAYS(DATEDIFF(CURRENT_DATE,B_DATE))) > 
...
SELECT EMPL_ID, YEAR(FROM_DAYS(DATEDIFF(CURRENT_DATE, START_DATE))) AS YEARS_SERVICE, 
(SELECT AVG(YEAR(FROM_DAYS(DATEDIFF(CURRENT_DATE, START_DATE)))) FROM JOB_HISTORY) 
AS AVG_YEARS_SERVICE
FROM JOB_HISTORY;